# 📙 Comparison Grid (Blank)  
**First created:** 2026-01-08 | **Last updated:** 2026-05-21  
*A printable grid for side-by-side comparison.*

---

Use this grid to line up responses from different sources.

You do not need to complete every cell.  
Blank space is information.

---

| Dimension | Controller (FOI / response) | Processor (vendor response) | SAR / lived data |
|---------|-----------------------------|-----------------------------|------------------|
| Role claimed | | | |
| Stated purpose | | | |
| Legal basis cited | | | |
| DPIA existence | | | |
| DPIA date | | | |
| DPIA scope | | | |
| Profiling acknowledged | | | |
| Inference / derived data | | | |
| Special category data | | | |
| Human review | | | |
| Automation claims | | | |
| Retention | | | |
| Data sharing | | | |
| Risk language | | | |
| Safeguards | | | |
| What is not mentioned | | | |

---

*This grid supports calm comparison, not argument.*

_Last updated: 2026-05-21_
