...
```mermaid
flowchart TD
    A[Latent vulnerability<br/>appointment, files, exposure, dependency] --> B[Disclosure pressure<br/>tranche, deadline, letter, leak]
    B --> C[Legitimacy dispute<br/>redactions, omissions, accountability claims]
    C --> D[State-system activation<br/>police, Parliament, Cabinet Office, FCDO, courts]
    D --> E[Transatlantic / institutional pressure<br/>US Congress, Humble Address, allied concern]
    E --> F[Defence / diplomatic compression<br/>bases, carriers, aircraft, posture, warnings]
    F --> G[Kinetic / attribution event<br/>strike, drone, operation, escalation]
    G --> H[Public framing problem<br/>denials, hedging, unclear attribution]
    H --> I[Legitimacy fallout<br/>sovereignty, trust, redactions, local anger]
    I --> J[Recurrence check<br/>next tranche, next deadline, next document release]
    J --> B
```
...
```mermaid
flowchart LR
    subgraph ATrack[Track A — Epstein Release Calendar]
        A1[Estate / DOJ / Oversight records]
        A2[Statutory deadlines]
        A3[Redaction disputes]
        A4[Mega-tranches]
    end

    subgraph BTrack[Track B — Mandelson State-System Risk]
        B1[Appointment due diligence]
        B2[Police review / arrest]
        B3[Humble Address]
        B4[US House request / response deadline]
        B5[UK document tranches]
    end

    subgraph CTrack[Track C — Andrew Comparator]
        C1[Royal / trade-envoy exposure]
        C2[Police action]
        C3[Government-record pressure]
        C4[Comparator testing]
    end

    subgraph DTrack[Track D — Defence Comms Gaza Iran]
        D1[Gaza ceasefire surface]
        D2[Russia anomaly]
        D3[US / Israel / UK posture]
        D4[Operation Epic Fury]
        D5[Akrotiri attribution]
        D6[Allied / local fallout]
    end
...
```mermaid
flowchart TD
    L[Legal-risk movement] --> M[Metadata shift]
    M --> N[Association graph changes]
    N --> O[Risk tools / AI indexing / adverse media]
    O --> P[Private defensive movement]
    P --> Q[Visible public signal]
    Q --> R[Adversary / ally / journalist interpretation]
    R --> S[Further defensive movement]
    S --> N
```
...
```mermaid
flowchart TD
    A[Legal / political pressure window] --> B[Operational movement]
    B --> C[Public explanation gap]
    C --> D[Attribution hedging]
    D --> E[Alliance-management pressure]
    E --> F[Domestic legitimacy pressure]
    F --> G[Further document / accountability demand]
    G --> A
```
...
```mermaid
flowchart TD
    A[Late-Feb legal compression<br/>Mandelson deadline / Andrew comparator / Epstein pressure] --> B[Operation Epic Fury]
    B --> C[RAF Akrotiri drone strike]
    C --> D[Attribution problem<br/>Iranian-made / proxy / launch-chain uncertainty]
    D --> E[UK public line<br/>no US bombers / no casualties / launch timing]
    E --> F[Cyprus sovereignty pressure]
    F --> G[Questions about UK exposure]
    G --> H[Renewed demand for clarity]
    H --> A
```
...
```mermaid
flowchart TD
    A[Possible early warning / exposure signal] --> B[Asset-protection movement]
    B --> C[Semi-visible aircraft / rumour field]
    C --> D[Weak or contested public confirmation]
    D --> E[Muted official propaganda]
    E --> F[Analytic question:<br/>why not cash it in?]
    F --> G[Hypotheses:<br/>source protection, dependency, leverage, mutual vulnerability]
    G --> H[Further date testing]
```
...
```mermaid
flowchart TD
    A[Iran escalation] --> B[Energy / oil risk]
    A --> C[Gulf shipping risk]
    A --> D[Sanctions / anti-US posture]
    A --> E[Strategic-partner support]
    B --> F[Conventional Chinese response]
    C --> F
    D --> F
    E --> F
    F --> G[Control case:<br/>legible state interest]
```
