...
```mermaid
flowchart LR
    A[Conduct producing harm]
    --> B[Civil-society objection]
    --> C[Economic / institutional pressure]
    --> D[Cost of continuing rises]
    --> E[Behaviour changes]
...
```mermaid
flowchart TD
    A[Occupation / dispossession / rights violations / civilian harm]
    --> B[Political and ethical objection]
    --> C[Boycott / divestment / sanctions]

    C --> D1[Commercial cost]
    C --> D2[Reputational cost]
    C --> D3[Institutional cost]
    C --> D4[Diplomatic cost]

    D1 --> E[Pressure to alter behaviour]
    D2 --> E
    D3 --> E
    D4 --> E
...
```mermaid
flowchart LR
    A[Witness civilian harm]
    --> B[Anger / grief / horror]
    --> C[Demand for action]
...
```mermaid
flowchart TD
    A[Identify potentially implicated supplier]
    --> B[Identify relevant products]
    --> C[Find clinically appropriate alternatives]
    --> D{Safe alternative for this patient?}

    D -->|Yes| E[Substitute where feasible]
    D -->|No| F[Use clinically necessary product]

    E --> G[Reduce economic participation]
...
```mermaid
flowchart TD
    A[Direct action / boycott / economic pressure]
    --> B{What establishes the target?}

    B -->|Jewish identity itself| C[Identity-targeted action]
