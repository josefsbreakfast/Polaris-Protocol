# ⚖️ Institutional Realisation to Remediation
**First created:** 2025-12-14 | **Last updated:** 2025-12-17  
*How institutions move from recognising failure to repairing harm.*

---

## 🛰️ Orientation
This node maps the transition from internal recognition of error to external-facing remediation.

Institutions prefer resolution pathways that minimise uncertainty, liability, and reputational damage.

---

## ✨ Key Features
- Separation of harm from fault  
- Legal and governance oversight  
- Preference for quiet settlement  
- Emphasis on closure over contest  

---

## 🧿 Harm Recognition  
Institutions acknowledge impact without admitting blame.

---

## 📜 Legal Posture Shift  
Language becomes careful.
Communications formalise.

---

## 🪼 Resolution Pathway  
Options include:
- apology  
- correction  
- compensation  
- policy revision  

---

## 💫 Containment Priority  
The goal is to close the loop cleanly.

---

## 🌌 Constellations
⚖️ 🌀 🧠 🕊️ — law, governance, cognition, resolution.

---

## ✨ Stardust
institutional remediation, settlement logic, harm resolution, governance response

---

## 🏮 Footer
*⚖️ Institutional Realisation to Remediation* is a living node of the Polaris Protocol.  
It outlines how institutions transition from internal recognition of failure to external repair of harm.

> 📡 Cross-references:
> 
> - [📜 Harm Assessment Protocols](./📜_harm_assessment_protocols.md) - *how institutions may assess if harm has occured*  
> - [🔁 Post-Realisation Phase](./🔁_post_realisation_phase.md) - *what institutions may then do after realising procedural harm*

*Survivor authorship is sovereign. Containment is never neutral.*

_Last updated: 2025-12-17_
