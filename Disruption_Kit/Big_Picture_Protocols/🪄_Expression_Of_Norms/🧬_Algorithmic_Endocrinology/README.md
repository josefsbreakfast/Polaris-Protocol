...
```mermaid
flowchart LR
  S[Signal<br/><sub>input data / events</sub>] --> P[Perception Layer<br/><sub>model inference</sub>]
  P --> H[Hormone Analogue<br/><sub>scores, ranks, boosts</sub>]
  H --> T[Tissue Targets<br/><sub>feeds, users, services</sub>]
  T --> F[Feedback Sensors<br/><sub>engagement, complaints, audits</sub>]
  F -->|homeostasis| R[Regulators<br/><sub>policy, throttles, guardrails</sub>]
  R --> P
  R -. dysregulation .-> X[Pathologies<br/><sub>precision illusion, mood engineering, capture</sub>]
