...
## 🛰️ Orientation
**Residual Attentional Conditioning Injury (RACI)** is a proposed *mechanism-first* construct describing how prolonged exposure to **unpredictable, salient stimuli** (including non-symbolic sensory “noise”) can produce **persistent attentional capture**, **hyperarousal**, and **physiological dysregulation** that *continues after the exposure ends*.

This node is written to:
- separate **mechanism** from **intent** (effects-first physiology),
- remain compatible with existing diagnostic frameworks without forcing equivalence,
- make room for **supporting and critical** research threads (including limitations and confounds).

---

## 🧾 Working definition (proposed)
**Residual Attentional Conditioning Injury (RACI):**  
A systems-induced injury state in which repeated unpredictable salience (digital, environmental, interpersonal, or sensory) produces persistent, involuntary **attentional orienting responses** and **hyperarousal**, with secondary disruption of sleep/circadian regulation and potential **direct neurometabolic effects**, despite intact insight and without requiring psychosis or fixed false beliefs.
...
## 🧬 Hypothesised pathophysiology (RACI model)
### A) Input: unpredictable salience
Examples:
- clustered/variable notifications, intermittent prompts, “timed” cues
- irregular sensory noise (auditory hum, intermittent vibration, low-frequency environmental noise)
- rotating stimulus modalities (“technique rotation”): alternating cue-types prevents habituation/extinction

**Key feature:** unpredictability + low controllability → persistent orienting.

### B) Core neurobiology: attentional capture as conditioned orienting
**Hypothesis:** repeated unpredictable salience produces *conditioned orienting responses* supported by:
- ↑ tonic LC–NE tone (sustained arousal; reduced extinction)
...
Noise/stress exposure may contribute to neurometabolic impacts via:
...
- variable schedules increase conditioning persistence
- prevents “predictable safe windows”
- keeps orienting responses broad and generalised

This is a learning-theory-consistent failure mode: *the system trains on stimulus salience itself* rather than extinguishing it.

---

## 🗂️ Comparisons to established ICD constructs (mapping, not equivalence)
...
### 3) Practical stance for clinicians
RACI may be best used as:
- a **formulation** (mechanism narrative), plus
- an ICD-coded diagnosis for service access (e.g., insomnia disorder, adjustment disorder, PTSD/CPTSD where criteria are met).
...
## 🧷 Research threads in critique / limitations (what could be wrong)
### 1) Construct proliferation / diagnostic inflation
Risk: RACI becomes “a new label for old phenomena” (insomnia, hyperarousal, PTSD-related hypervigilance).  
Mitigation: keep RACI as **mechanism** + **formulation tool**, not necessarily a standalone disorder.

### 2) Causality and confounds
- Noise exposure, trauma history, disability stressors, and administrative uncertainty are highly confounded in real life.
- Many studies are observational; attribution requires careful design and/or within-person longitudinal measures.

### 3) Measurement problem (operationalising RACI)
RACI requires measurable markers beyond self-report:
- attentional capture metrics (e.g., vigilance tasks; attentional bias paradigms)
- autonomic markers (HRV patterns; nocturnal sympathetic surges)
- endocrine rhythm (diurnal cortisol slope; evening cortisol)
- sleep architecture (micro-arousals; fragmentation; REM/NREM disruption)

Without measurement consensus, RACI risks becoming rhetorically powerful but empirically mushy.

### 4) Individual differences
Baseline trauma exposure, neurodivergence, sleep disorders, medication effects, and working memory capacity can moderate susceptibility—potentially making RACI heterogeneous.
...
## 🧭 Open questions
- What is the minimal exposure duration/intensity to induce persistent residual conditioning?
- Which biomarkers best distinguish RACI from PTSD hypervigilance vs primary insomnia?
- Can re-entrainment protocols (sleep timing, light exposure, autonomic stabilisation) accelerate extinction in RACI?
- How does “technique rotation” alter extinction kinetics and generalisation?

---

## 🧯 Evidence-based risk reduction (RACI harm minimisation)
**Scope:** practical steps that reduce (a) conditioned orienting / hyperarousal, (b) circadian disruption, and (c) downstream neurometabolic load — without requiring attribution of intent.

### 1) Reduce unpredictability at the stimulus layer (highest leverage)
RACI is amplified by *unpredictable, uncontrollable salience*. Primary goal: convert “random hits” into “bounded signals”.
- **Notification triage:** disable non-essential push notifications; batch delivery where possible; whitelist only user-set alarms/reminders.
- **Time-bounding:** create predictable “quiet windows” (e.g., 90–120 min blocks) to permit extinction learning.
...
### 5) Autonomic downshift training (physiology-first)
Because micro-arousals and autonomic shifts can occur without full waking, downshifting sympathetic tone is often helpful.
- daily paced breathing / HRV biofeedback style practice (short, regular doses)
...
## 🧾 Investigator / clinician ethics reminder (non-extractive handling)
**If any investigation, safeguarding process, or service response is underway:**
- **No new “extractive research” on the subject** (no experimental probing, no additional behavioural manipulation, no “testing responses”).
- **Consent boundary:** use **pre-existing records only** unless the subject gives explicit, informed, revocable consent for new data collection.
- **Minimise collection:** collect the smallest dataset necessary; time-bound retention; document access.
- **Duty of closure:** provide *status updates within reason* (even if details can’t be disclosed), because prolonged uncertainty is itself a harm multiplier in RACI trajectories.
- **Do not pathologise reality-testing:** persistent interrogation of salience can be a conditioned attentional effect, not delusion.

*Operational note:* This section exists because vulnerable populations require **higher** ethical standards, and because RACI can be worsened by continued uncertainty, stimulus rotation, or unannounced technique changes.

---

## 🌌 Constellations
🧠 🧬 🩸 💤 🛰️ — mechanistic cognition; neurobiology; endocrine/metabolic load; sleep/circadian disruption; systems/inputs and monitoring.

## ✨ Stardust
residual attentional conditioning injury, rাকি, locus coeruleus, norepinephrine, hpa axis, cortisol slope, micro-arousals, noise exposure, insomnia disorder, allostatic load, circadian desynchronisation, hyperarousal, attentional capture

---

## 🏮 Footer
*🧠 Residual Attentional Conditioning Injury* is a living node of the Polaris Protocol.  
