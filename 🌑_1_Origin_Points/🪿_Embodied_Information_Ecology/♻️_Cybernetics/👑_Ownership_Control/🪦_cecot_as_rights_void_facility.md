...
```mermaid
flowchart TD
    A["Sending state selects and transfers"] --> B["Receiving state confines"]
    B --> C["Access, counsel and monitoring narrow"]
    C --> D["Each actor disputes remedial control"]
    D --> E["Court or advocate reaches only part of chain"]
    E --> F["Delay allows harm to continue"]
    F --> C
```
