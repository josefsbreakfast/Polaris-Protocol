...
```mermaid
flowchart TD
  H[⚠️ Harm or control failure] --> A[🏛️ Public authority]
  H --> V[💼 Vendor / prime contractor]
  H --> R[⚖️ Regulator or oversight body]

  A -->|contract boundary| V
  V -->|subcontract + data boundary| S[🧩 Subcontractor / system component]
  A -->|limited statutory remit| R

  A --> D[🛡️ Protect service continuity]
  V --> D
  R --> E[🔍 Apply evidence + jurisdiction threshold]
  E -->|threshold not met or only partial remedy| F[🧱 Fragmented consequence]
  D -->|removal appears operationally dangerous| F

  F -->|harm persists but no actor owns integration| H
  F --> N[📄 Local compliance narratives remain available]
  N -->|vendor: contract / authority: reliance / regulator: remit| F
```
