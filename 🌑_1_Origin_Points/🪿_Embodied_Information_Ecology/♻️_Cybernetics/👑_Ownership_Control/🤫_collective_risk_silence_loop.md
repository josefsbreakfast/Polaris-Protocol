...
```mermaid
flowchart TD
  S[⚠️ Ambiguous but credible risk signal] --> A[🏛️ Institution A assesses its slice]
  S --> B[🏢 Institution B assesses its slice]
  S --> C[⚖️ Oversight body assesses remit + threshold]

  A -->|waits for confirmation| O[👀 Observes the others]
  B -->|avoids unilateral exposure| O
  C -->|acts narrowly or seeks more evidence| O

  O -->|nobody moves visibly| N[🤐 Silence appears normal and prudent]
  N -->|staff infer others see no actionable problem| U[📉 Perceived urgency falls]
  U -->|less reporting + less shared evidence| E[🧩 Evidence remains fragmented]
  E -->|thresholds remain unmet| O

  E --> H[🧨 Risk or harm persists]
  H -->|new complaint, incident, audit or inquiry| S
```
