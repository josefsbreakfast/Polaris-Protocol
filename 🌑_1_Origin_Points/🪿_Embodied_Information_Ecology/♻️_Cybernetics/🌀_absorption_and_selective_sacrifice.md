...
```mermaid
flowchart TD
    A[Evidence of harm] --> B[Public pressure]

    B --> C1[Media attention]
    B --> C2[Political criticism]
    B --> C3[Campaign activity]
    B --> C4[Institutional review]

    C1 --> D[Highly salient target]
    C2 --> D
    C3 --> D
    C4 --> D

    D --> E[Target absorbs pressure]
    ...
    I --> J[Original incentives persist]
```
...
```mermaid
flowchart TD
    X[Observed systemic problem]

    X --> A[Actor A]
    X --> B[Actor B]
    X --> C[Actor C]
    X --> S[Structural conditions]

    A --> P[Corrective pressure]
    B --> P
    C --> P
    S --> P
```
...
flowchart LR
    A[Systemic problem]
    --> B[Corrective pressure]
    --> C[Selective salience]
    --> D[Selective legitimation]
    --> E[Asymmetric accountability]
    --> F[Selective sacrifice]
    --> G[Visible correction]
...
```mermaid
flowchart TD
    M[Media wants legible story] --> A[Attention concentrates]
    P[Politicians want manageable response] --> A
...
```mermaid
flowchart TD
    A[System produces harm]
    --> B[Harm generates criticism]
    --> C[Criticism threatens legitimacy]
    --> D[Bounded accountability response]
    --> E[Legitimacy partially restored]
    --> F[Underlying architecture retained]
    --> A
```
...
```mermaid
flowchart TD
    A[Corrective pressure] --> B{Response}

    B --> C[Performance of correction]
    B --> D[Transmission of correction]

    C --> C1[Visible sanction]
    C1 --> C2[Legitimacy restored]
    C2 --> C3[Reproducing conditions persist]

    D --> D1[Incentives change]
    D1 --> D2[Dependencies change]
    D2 --> D3[Behaviour changes]
