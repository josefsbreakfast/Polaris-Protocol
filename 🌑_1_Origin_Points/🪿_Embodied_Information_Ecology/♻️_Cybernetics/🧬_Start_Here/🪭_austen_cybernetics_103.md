...
flowchart TD
    ...
    C --> D[Elizabeth answers and refuses]
    D --> E[Lady Catherine's interpretation]
    E --> F[Lady Catherine reports to Darcy]
    F --> G[Darcy's different model]
    G --> H["It taught me to hope"]
    H --> I[Second proposal]
...
flowchart TD
    ...
    C --> D[Diagnostic negative feedback]
    D --> E[Darcy processes and revises]
    E --> F[Changed behaviour at Pemberley]
    F --> G[Elizabeth receives new observations]
    G --> H[Elizabeth updates too]
    H -. further feedback .-> E
```
...
flowchart TD
    ...
    A --> B[Lady Catherine]
    A --> C[Darcy]
    A --> D[Other possible observers]
    B --> B1[perverse / assured / inferior]
    C --> C1[hope / integrity / courage / 🥵]
    D --> D1[headstrong? audacious? impressive? scandalous?]
```
...
```mermaid
flowchart TD
    A[Question asked] --> B[Indirect answer / non-disclosure]
    B --> C1[Legal or policy constraint]
    B --> C2[Confidentiality]
    B --> C3[Third-party dignity]
    B --> C4[Self-dignity]
    B --> C5[Politeness / face-saving]
    B --> C6[Relational boundary]
    B --> C7[Discomfort]
    B --> C8[Actual concealment]
```
...
```mermaid
flowchart TD
    A[Constraint layer<br/>What can actually be disclosed?]
    --> B[Propositional layer<br/>What is explicitly said?]
    --> C[Relational layer<br/>What does the formulation signal about the relationship?]
    --> D[Cultural layer<br/>What conventions does the sender expect to be recognised?]
    --> E[Receiver model<br/>What does the receiver think those conventions mean?]
    --> F[Inference<br/>Why do I think they refused?]
```
...
flowchart TD
    ...
    C --> D[Collins / Rosings network]
    D --> E[Lady Catherine: URGENT THREAT]
    E --> F[Longbourn intervention]
...
flowchart TD
    ...
    C --> D[Darcy letter]
    D --> E[Elizabeth updates]
    C --> F[Darcy processes criticism]
    F --> G[Changed behaviour at Pemberley]
    G --> E
    E --> H[Further mutual observations]
    H --> I[Lydia crisis / Darcy intervention]
    I --> J[Elizabeth learns more]
    J --> K[Lady Catherine intervenes]
    K --> L[Elizabeth refuses control]
    L --> M[Lady Catherine reports to Darcy]
    M --> N["It taught me to hope"]
    N --> O[Carefully defeasible query]
    O --> P[Elizabeth's authoritative answer]
    P --> Q["💍 SYSTEM CONVERGES"]
```
