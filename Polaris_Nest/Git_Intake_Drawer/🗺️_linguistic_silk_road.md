...
```mermaid
flowchart LR
  A[Proto-Semitic<br>Ancient Near East] --> B[Arabic]
  B --> C[Persian]
  C --> D[Urdu]
  D --> E[Hindi]
  E --> F[English]
  F -->|loanwords| G[Global Internet English]
  ...
  subgraph IndianOcean
    K[Tamil / Malay / Swahili] --> L[Portuguese]
    L --> F
  end
  
  subgraph Atlantic
    M[West African Languages] --> N[Caribbean Creoles]
    N --> F
  end
```
