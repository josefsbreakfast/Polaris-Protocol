...
flowchart TD
    ...
    --> B[Activity]
    --> C[Harm]
    --> D[Harm becomes legible]
    --> E[Cost returns toward decision-maker]
...
flowchart LR
    ...
    --> B[Investment]
    --> C[Dependency]
    --> D[Switching becomes expensive]
    --> E[Existing choice appears inevitable]
    --> F[More investment]
    --> C
```
...
flowchart TD
    ...
    A --> E[Changed incentive environment]
    B --> E
    C --> E
    D --> E

    E --> F[Different behaviour becomes rational]
```
...
```mermaid
flowchart TD
    A[Potential harmful behaviour]
    --> B[Detected]
    --> C[Relevant costs activate]
    --> D[Decision-maker recalculates]

    D --> E{Continue?}

    E -->|Yes| F[Further cost / constraint]
    F --> D
...
```mermaid
flowchart LR
    A[Protect marginal private return]
    --> B[Resist corrective reform]
    --> C[Instability accumulates]
    --> D[Larger crisis]
