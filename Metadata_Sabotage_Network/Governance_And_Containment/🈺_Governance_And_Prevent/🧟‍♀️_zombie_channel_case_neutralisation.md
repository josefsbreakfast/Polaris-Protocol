...
flowchart TD
    ...
    A[Referral Received<br>• Often weak or ambiguous<br>• Politically or socially charged] 
        --> B[Initial Triage by PREVENT & CTU<br>• No clear risk<br>• No clear non-risk]

    B --> C{Does the case fit a known extremism category?}
    C -- Yes --> D[Short Channel Intervention]
    C -- No --> E[Classification Paralysis<br>• Can't label Islamist/Far Right/Mixed/Single]

    D --> F[Case Usually Closes]

    E --> G[CTU Residual Risk Anxiety<br>“Not enough to close, not enough to escalate”]

    G --> H[Multi-Agency Loop<br>• LA wants CTU decision<br>• CTU wants NHS clarity<br>• NHS wants LA clarity]
    H --> I[Decision Paralysis]

    I --> J{Any partner confident enough to sign closure?}
    J -- Yes, rare --> F
    J -- No (common) --> K[Case Renewed]

    K --> L[“Soft Surveillance” Mode<br>• Passive monitoring<br>• No active intervention]

    L --> M{Any life event with new data?<br>e.g., new GP, job, uni, move}
    M -- Yes --> N[Fresh Data Pulls<br>Renewed requests for information]
    M -- No --> O[Low-Intensity Stagnation]

    N --> P[Perceived Complexity Increases<br>• CTU: “We still have concerns”]
    P --> K

    O --> Q[Metrics Incentive<br>• Case counts as “active”]
    Q --> R[Cover-Your-Arse Logic<br>“Safer to keep open than close”]

    R --> S[Zombie Case<br>• No risk evidence<br>• No intervention<br>• No exit pathway]

    S --> K[→ loops back into renewal cycle]
```
