...
```mermaid
flowchart LR
    A["Original Note<br>(subjective / contextual)"] --> B["Data Export<br>(summary / flag)"]
    B --> C["Cross-System Interpretation<br>(risk inflation)"]
    C --> D["Repetition in New Records"]
    D --> E["Assumed Validation<br>('multiple systems flagged this')"]
    E --> F["Institutional Behaviour Change<br>(heightened caution)"]
```
