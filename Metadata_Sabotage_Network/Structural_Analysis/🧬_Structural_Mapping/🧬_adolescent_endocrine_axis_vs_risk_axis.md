...
```mermaid
graph TD
  A[Neuro-Endocrine Axis] --> B[Hormonal Pulses]
  B --> C[Neural Plasticity & Mood Flux]
  C --> D[Boundary Testing & Social Feedback]
  D --> E[Calibration → Emotional Maturity]

  X[Prevent / Risk Axis] --> Y[Behavioural Capture]
  Y --> Z[Sentiment Analysis / Flag]
  Z --> W[Institutional Intervention]
  W --> V[Internalised Surveillance / Self-Censorship]
  V --> Y

  C -. physiological volatility misread as anomaly .-> Y
  D -. boundary testing reclassified as instability .-> Z

  style A fill:#eaf7ff,stroke:#6ca,stroke-width:2px
  style X fill:#ffeaea,stroke:#c66,stroke-width:2px
```
