...
```mermaid
flowchart LR
A["Input error (E₀)"] --> B["Replication cycles (r)"]
B --> C["Propagation factor (p)"]
C --> D["Error persistence (Eₙ = E₀ × pʳ)"]
D --> E["Decay interventions reduce p"]
E --> F["Residual belief persists beyond data correction"]
```
