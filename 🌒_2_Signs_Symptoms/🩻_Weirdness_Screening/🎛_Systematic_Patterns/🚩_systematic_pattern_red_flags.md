...
```mermaid
flowchart TD
    A[Repeated issue] --> B{Any red flags?}

    B -->|No| C[Ordinary or low concern]
    B -->|Yes| D{High-stakes impact?}

    D -->|Yes| E[Preserve evidence and use alternate route]
    E --> F[Escalate cleanly]

    D -->|No| G{How specific is recurrence?}

    G -->|Vague| H[Log lightly and compare]
    G -->|Specific| I{Survives basic comparison?}

    I -->|No| J[Likely ordinary/local issue]
    I -->|Yes| K[Pattern suspected]

    K --> L[Timeline overlay + artifacts]
    F --> L
```
