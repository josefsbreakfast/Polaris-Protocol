...
```mermaid
flowchart TD
    A[Something weird happened] --> B{Did it affect safety, money, legal, medical, or essential access?}
    B -->|Yes| C[Preserve evidence and escalate promptly]
    B -->|No| D{Did it resolve after one obvious fix?}
    D -->|Yes| E[Ordinary glitch: note only if useful]
    D -->|No| F{Can you reproduce it?}
    F -->|No| G[Make brief log and watch for recurrence]
    F -->|Yes| H{Does it persist across device, browser, network, or account?}
    H -->|No| I[Local issue likely: troubleshoot specific layer]
    H -->|Yes| J{Does it cluster around sensitive content, deadlines, contacts, or escalation?}
    J -->|No| K[Structured log: monitor and compare]
    J -->|Yes| L[Pattern suspected: preserve artifacts and escalate cleanly]
```
...
```mermaid
flowchart TD
    A[🩻 Weirdness Screening] --> B[🌐 Connection Hiccups]
    A --> C[🎛 Systematic Patterns]
    A --> D[📂 Data Shifts]
    A --> E[📬 Comms Breaks]
    A --> F[🔑 Access Barriers]
    A --> G[🖥 Interface Glitches]
    A --> H[🚉 Infrastructure Hiccups]
    A --> I[🛒 Service Blockages]

    B --> J[Network, upload, router, signal]
    C --> K[Recurrence, timing, clustering]
    D --> L[Files, metadata, records]
    E --> M[Messages, calls, attachments]
    F --> N[Login, MFA, submit, permission]
    G --> O[Buttons, fields, cursor, UI]
    H --> P[Transport, banking, utilities]
    I --> Q[Shopping, checkout, booking, payment]
```
