...
```mermaid
flowchart TD
    A[Something weird has happened] --> B{Is there a specific object at the centre?}

    B -->|Yes| C[🎞️ Haunted Artefact]
    B -->|No / not yet| D{Does it feel like a recurrence, presence, echo, or misrecognition?}

    C --> C1[File, image, screenshot, record, dataset, report, attachment, broadcast, contract, archive fragment]
    C1 --> C2[Ask: what object carries the trace?]
    C2 --> C3[Log in 🎞️_Haunted_Artefacts_Catalogue]

    D -->|Yes| E[👻 Glitchy Ghostie]
    D -->|No / unclear| F{Has a process, behaviour, interface, or institution split, drifted, duplicated, reverted, or degraded?}

    E --> E1[System seems to remember, infer, resurface, follow, recognise, or echo]
    E1 --> E2[Ask: what appeared, where, and under what trigger conditions?]
    E2 --> E3[Log in 👻_Glitchy_Ghosties]

    F -->|Yes| G[🍽️ Fork]
    F -->|No / unclear| H[🩻 Weirdness Screening]

    G --> G1[Pattern of divergence, clone-like mismatch, execution drift, sanitising, reversion, repetition, or degradation]
    G1 --> G2[Ask: what behaviour changed from the expected path?]
    G2 --> G3[Log in 🍽️_Fork_Taxonomy]

    H --> H1[Hold as unresolved weirdness]
    H1 --> H2[Gather timestamp, surface, evidence, user effect, and confidence level]
    H2 --> H3[Reclassify later if the pattern becomes clearer]

    C3 --> I{Do other categories also apply?}
    E3 --> I
    G3 --> I
    H3 --> I

    I -->|Yes| J[Cross-reference secondary category]
    I -->|No| K[Keep primary classification]

    J --> L[Separate object, behaviour, feeling, and interpretation]
    K --> L

    L --> M[Final check: what is known, inferred, uncertain, or speculative?]
```
